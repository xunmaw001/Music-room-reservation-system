...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussleqizhishi');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','discussleqizhishi');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('discussleqizhishi', this.id);
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discussdiscussleqizhishi', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussdiscussleqizhishi/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('discussleqizhishi', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('discussleqizhishi', this.detail);
				this.$utils.msg('审核成功');

      ...
      <el-row>
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.tupian" label="图片" prop="tupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.tupian.split(',')" :src="item" width="100" height="100">
                                    ...
                                    <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="资料附件" prop="ziliaofujian">
          <file-upload
          tip="点击上传资料附件"
          action="file/upload"
          :limit="1"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.ziliaofujian" label="资料附件" prop="ziliaofujian">
            <el-button type="text" size="small" @click="download(ruleForm.ziliaofujian)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
                                                            </el-row>
                                                                                                                                                            <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="乐器知识" prop="leqizhishi">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.leqizhishi" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.leqizhishi" label="乐器知识" prop="leqizhishi">
                    <span v-html="ruleForm.leqizhishi"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    onSubmit() {
        ...
        if (valid) {
          this.$http({
            url: `leqizhishi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.leqizhishiCrossAddOrUpdateFlag = false;
                  this.parent.search();
